...
#AUC
#####################################
AUCFunction = function(specificity, Sensitivity) {
...
# ROC Curve
###########################
RocCurveFunction = function(specificity, Sensitivity) {
    thresh=seq(0.01,1,0.01)
    xval=1-specificity
    yval=Sensitivity
...
fold=as.numeric(Sys.getenv("FOLD")) # -1 for leave-one out
method=Sys.getenv("METHOD")
nested=Sys.getenv("NESTED") # NO and YES
NumberOfIterations=as.numeric(Sys.getenv("ITERATIONS"))
BootStrap=as.numeric(Sys.getenv("BOOTSTRAP_ITERATIONS"))
TransformFunction=Sys.getenv("TRANSFORM")   # (NA, log, log2. log10, exp)maxBiomarkers=Sys.getenv("MAXBIOMARKERS")
scale=Sys.getenv("SCALE") # none, row, col

filename<-Sys.getenv("FPKM_MATRIX")
cummerbundDir=Sys.getenv("CUMMERBUND_OUTPUT")
dirpath<-paste0(cummerbundDir,"/",diffDir)
fpkmMatrix<-paste0(dirpath,"/",filename)

TopRocCurve=as.numeric(Sys.getenv("TOPROCCURVE"))
delta=as.numeric(Sys.getenv("DELTA"))
print(paste0("delta=",delta))

Output<-outDir
data<-read.table(fpkmMatrix)
...
if (NumberOfFolds>0) {
    ...
    if (nested=="YES") {
        ...
        Output=paste0(Output,"Nfold/")

    dir.create(Output, showWarnings = TRUE, recursive = FALSE, mode = "0777")
}

ROCCurveMatrix=matrix(0,ncol=4,nrow=101)
AUCValue=matrix(0,ncol=2,nrow=length(ALphaList))
colnames(AUCValue)=c("Alpha","AUC")
    ...
    if (NumberOfFolds>0)
        {
            if (nested=="YES") {
                ...
                ROCCurveMatrix[is.na(ROCCurveMatrix)]=0
                SensitivityVector=c()
                for (k in 1:101)
                    {
                ...
                AUCValue[Index,]=c( i, AUCFunction(specificity=specificityVector,Sensitivity=SensitivityVector))
                write.csv(rbind(SensitivityVector,specificityVector), file=paste0(Output,"Alpha_",i,"_RocCurveValue.csv"))
...
write.csv(AUCValue, file=paste0(Output,"OveralAUC.csv"))
...
for (alpha in ALphaList) {
    ...
    heatmap.2(t(MarkerMatrix), col=redgreen(75), scale=scale, dendrogram="col",
              key=TRUE, symkey=FALSE, density.info="none", trace="none", cexRow=0.5)
                                        #heatmap(t(MarkerMatrix),Rowv=NA, scale=scale, margins=c(5,10),na.rm = T)
...
############################
## Generate RocCurves from matrix
############################
TopAUCSorted=order(AUCValue[,2],decreasing = T)
TopRocCurve=ifelse(TopRocCurve>length(TopAUCSorted),length(TopAUCSorted),TopRocCurve)

for (z in 1:TopRocCurve) {

# Read the non-zero genes and count them.

    setwd(Output)
    FileName=paste0(Output,"Alpha_",AUCValue[TopAUCSorted[z],1],"_RocCurveValue.csv")
    ROCValue=read.csv(FileName, header = T,row.names = 1)
...
############################
    thresh=seq(0.01,1,0.01)
    xval=unlist(1-ROCValue[2,])
    yval=unlist(ROCValue[1,])
    ...
    my.auc = min(xydf$xval)*min(xydf$yval)/2 +  #area below the minimum y value
        sum(b*xydf$yval[-length(xydf$yval)])+(1-max(xydf$xval))*max(xydf$yval) +  #area of rectangles and triangles
            ((1-max(xydf$xval))*(1-max(xydf$yval))/2) #area after the max y value
                                        #pdf(file =paste0(fileName,".pdf"))
    if (z==1)
        plot(yval~xval, xydf, type="l", xlab="1-Specificity", ylab="Sensitivity", main="ROC Curve", col=z)
    else
        points(yval~xval, xydf, type="l", col=z)
