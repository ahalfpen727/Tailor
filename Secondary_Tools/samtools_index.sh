...
PROCESSORS=4
MEMORY="8096"      # PER PROCESSOR!! - 2048=2G, 4096=4G, 8192=8G, 16384=16G, 32768=32G, 65536=64G
...
while IFS='' read -r line || [[ -n "$line" ]]; do   # each line is a directory that contains a accepted_hit.bam file
    ...
    if [[ $line =~ $SAMTOOLS_INPUT_REGEX ]]; then
        ...
        COMMAND_LINE="${COMMAND} index ${INPUT}/${SAMPLE}_out/accepted_hits.bam  ${INPUT}/${SAMPLE}_out/accepted_hits.bai"
