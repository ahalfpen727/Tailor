...
#--------------------------------------------------------------------------------------------------------------
#  launchR.Classification.sh - launches R scripts for cummerbund output 
#
...
INPUT="${LAUNCHR_CLASSIFICATION_INPUT}"
...
while IFS='' read -r line || [[ -n "$line" ]]; do   # each line is a cuffdiff output directory
    ...
    COMMAND_LINE="${XVFB_RUN} ${COMMAND} CMD BATCH --no-restore '--args inFile=\"${diffFile}\" inDir=\"${INPUT}/${overDir}\" outDir=\"${OUTPUT}/${overDir}/${diffDir}\" over=\"${over}\" under=\"${under}\"' ${TAILOR}/${1}.R ${OUTPUT}/${overDir}/${diffDir}/${1}.Rout"
