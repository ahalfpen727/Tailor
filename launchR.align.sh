...
DURATION="96:00"    # HH:MM - 4:00=short queue limit, 72:00=3 days, 96:00=4 days, 192:00=8 days, 384:00=16 days
...
while IFS='' read -r line || [[ -n "$line" ]]; do   # each line is a cuffdiff output directory
    ...
    diffDir="" # strip off the ".diff" extension
    ...
    COMMAND_LINE="${XVFB_RUN} ${COMMAND} CMD BATCH --no-restore '--args inFile=\"${line}\" inDir=\"${INPUT}/${overDir}\" outDir=\"${OUTPUT}\" over=\"${over}\" under=\"${under}\"' ${TAILOR}/${1}.R ${OUTPUT}/${1}.Rout"
