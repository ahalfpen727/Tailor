...
#--------------------------------------------------------------------------------------------------------------
# Cufflinks - quantifies gene expression for each sample
#
...
PROCESSORS=7
MEMORY="10012"      # PER PROCESSOR!! - 2048=2G, 4096=4G, 8192=8G, 16384=16G, 32768=32G, 65536=64G
...
#findResults=${OUTPUT}/threeFailed.list.txt
 find ${INPUT} -type d -maxdepth 1 -name "${CUFFLINKS_FIND_INPUT_PATTERN}" > $findResults  # search for only directories and no subdirs
#----------------------------------------------
...
while IFS='' read -r line || [[ -n "$line" ]]; do   # each line is a directory that contains a accepted_hit.bam file
    ...
    if [[ $fileName =~ $CUFFLINKS_INPUT_REGEX ]]; then
